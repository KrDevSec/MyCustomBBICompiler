...
int MyPower(int d1, int d2)	/* 제곱을 구하는 함수 */
{
	int nResult = 1;
	for(int i=0; i<d2; i++)
		nResult *= d1;
...
int MyGCD(int d1, int d2) /* 팩토리얼을 구하는 함수 */
{
	if( d2 <= 0 )
		return d1;
	return MyGCD(d2, d1%d2);
}
...
{
    switch (kd) {
	case Multi: case Divi: case Mod: case Power: case GCD: // custom '_', '~'
	...
	case And:	case ExclusiveOr:    return 2; /* && custom '^'   */
    case Or:                         return 1; /* ||         */
